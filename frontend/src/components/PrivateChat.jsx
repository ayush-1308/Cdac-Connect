        ...
        const loadMessageHistory = async () => {
            try {
                const response = await fetch(
                    `${import.meta.env.VITE_BACKEND_API_URL}/api/messages/private?user1=${currentUser}&user2=${recipientUser}`
                );
        ...
        if (message.trim() && stompClient.current && stompClient.current.connected) {
            ...
            try {
                if (stompClient.current.connected) {
                    // Use publish() method instead of send() for the new STOMP client API
